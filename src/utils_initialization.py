...
def initialize_model(params, dl, model, load_model=False):
    if load_model and os.path.exists(os.path.join('checkpoints', 'pgrail_'+params.save_str+'.pt')):
        logging.info('Loading existing model from %s' % os.path.join('checkpoints', 'pgrail_'+params.save_str+'.pt'))
        graph_classifier = torch.load(os.path.join('checkpoints', 'pgrail_'+params.save_str+'.pt')).to(device=params.device)
    else:
        logging.info('No existing model found. Initializing new model..')
        graph_classifier = model(params, dl).to(device=params.device)
